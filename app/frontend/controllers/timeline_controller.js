...
export default class extends Controller {
  ...
  updateSelectedEvent() {
    this.eventTargets.forEach(eventElement => {
      const isSelected = eventElement.dataset.eventId === this.selectedEventIdValue;
      
      if (isSelected) {
        eventElement.classList.remove('bg-gray-200/90');
        ...
        eventElement.classList.add('bg-gray-200/90');
  ...
  calculateTimelinePosition(targetDate) {
    ...
    for (let i = 0; i < monthElements.length; i++) {
      ...
      if (targetDate >= monthDate && targetDate < nextMonthDate) {
        ...
        if (window.timelineDebug && (dateStr === '2024-02-22' || dateStr === '2021-02-25')) {
